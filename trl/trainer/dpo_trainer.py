...
class DataCollatorForPreference(DataCollatorMixin):
    ...
    def torch_call(self, examples: list[list[int] | Any | dict[str, Any]]) -> dict[str, Any]:
        ...
        # Add consistency weight field if present (for RGPO)
        if "consistency_weight" in examples[0]:
            output["consistency_weight"] = torch.tensor([example["consistency_weight"] for example in examples])
        if "s_i" in examples[0]:
            output["s_i"] = torch.tensor([example["s_i"] for example in examples])
...
class DPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        # RGPO (Reliability-Guided Preference Optimization) with Dawid-Skene model
        self.use_consistency_weighted = args.use_consistency_weighted
    ...
    def tokenize_row(
        ...
        # Preserve consistency weight field if present (for RGPO)
        if "consistency_weight" in features:
            result["consistency_weight"] = features["consistency_weight"]
        if "s_i" in features:
            result["s_i"] = features["s_i"]
    ...
    def process_row(
        ...
        # Preserve consistency weight field if present (for RGPO)
        if "consistency_weight" in features:
            output["consistency_weight"] = features["consistency_weight"]
        if "s_i" in features:
            output["s_i"] = features["s_i"]
    ...
    def dpo_loss(
        ...
        sample_weights: torch.FloatTensor | None = None,
        chosen_lengths: torch.FloatTensor | None = None,
        rejected_lengths: torch.FloatTensor | None = None,
        ...
        Args:
            ...
            loss_type (`str`, defaults to `"sigmoid"`):
                ...
                - `"ipo"`: IPO loss from the [IPO](https://huggingface.co/papers/2310.12036) paper.
                  The loss is: (log_ratio_chosen - log_ratio_rejected - 1/(2*beta))^2.
                  Can be combined with `use_consistency_weighted=True` to apply Dawid-Skene sample weights:
                  |s_i| * (log_ratio - 1/(2*beta))^2.
                ...
                - `"simpo"`: SimPO loss from the [SimPO](https://arxiv.org/abs/2405.14734) paper. Uses length-normalized
                  log probabilities without a reference model. Can be combined with `use_consistency_weighted=True`
                  to apply Dawid-Skene sample weights for RGPO.
            ...
            chosen_lengths (`torch.FloatTensor`, *optional*):
                Lengths of chosen responses (number of tokens). Required for SimPO loss.
            rejected_lengths (`torch.FloatTensor`, *optional*):
                Lengths of rejected responses (number of tokens). Required for SimPO loss.
        ...
        # In margin mode, we now use the standard DPO formula with reference model
        ...
        if loss_type == "sigmoid":
            ...
            # L_IPO = E[(log π_θ(y_w|x)/π_ref(y_w|x) - log π_θ(y_l|x)/π_ref(y_l|x) - 1/(2β))^2]
            # When use_consistency_weighted=True, sample_weights |s_i| are applied at the end of this function:
            # L_RGPO_IPO = E[|s_i| * (log_ratio - 1/(2β))^2]
            ...
            # SimPO loss from https://arxiv.org/abs/2405.14734
            # L_SimPO = -E[log σ(β/|y_w| * log π(y_w|x) - β/|y_l| * log π(y_l|x) - γ)]
            # SimPO is reference-free and uses length-normalized log probabilities
            if chosen_lengths is None or rejected_lengths is None:
                raise ValueError("SimPO loss requires chosen_lengths and rejected_lengths to be provided.")

            # Compute length-normalized log probabilities
            # chosen_logps and rejected_logps are already the sum of log probs over tokens
            chosen_lengths_float = chosen_lengths.to(device).float().clamp(min=1.0)
            rejected_lengths_float = rejected_lengths.to(device).float().clamp(min=1.0)

            # Length-normalized log probs: log π(y|x) / |y|
            chosen_logps_normalized = chosen_logps.to(device) / chosen_lengths_float
            rejected_logps_normalized = rejected_logps.to(device) / rejected_lengths_float

            # SimPO logits: β * (normalized_chosen - normalized_rejected) - γ
            simpo_logits = self.beta * (chosen_logps_normalized - rejected_logps_normalized) - self.args.simpo_gamma

            # SimPO loss: -log σ(simpo_logits)
            losses = -F.logsigmoid(simpo_logits)

            # For SimPO, rewards are based on length-normalized log probs (no reference model)
            chosen_rewards = (self.beta * chosen_logps_normalized).detach()
            rejected_rewards = (self.beta * rejected_logps_normalized).detach()
        ...
        # Compute rewards using standard DPO formula (with reference model)
        # Skip for SimPO which uses length-normalized rewards computed above
        if loss_type != "simpo":
            chosen_rewards = self.beta * (chosen_logps.to(device) - ref_chosen_logps.to(device)).detach()
            rejected_rewards = self.beta * (rejected_logps.to(device) - ref_rejected_logps.to(device)).detach()

        # Apply sample weights if provided (for robust weighted DPO)
        if sample_weights is not None:
            losses = losses * sample_weights.to(device)
    ...
    def concatenated_forward(
        ...
        # Compute completion lengths for SimPO (length-normalized log probs)
        completion_lengths = loss_mask.sum(dim=1)
    ...
    def get_batch_loss_metrics(
        ...
        # Handle RGPO: get consistency weights
        sample_weights = None
        if self.use_consistency_weighted and "consistency_weight" in batch:
            sample_weights = batch["consistency_weight"]
        ...
        if self.args.use_liger_kernel:
            ...
            # Apply weights if using robust weighted DPO
            if sample_weights is not None:
            ...
            # Get completion lengths for SimPO loss types
        ...
        # Both standard and margin modes now use rewards relative to reference model
        metrics_chosen_rewards = chosen_rewards
        metrics_rejected_rewards = rejected_rewards

        reward_accuracies = (metrics_chosen_rewards > metrics_rejected_rewards).float()
        ...
        metrics[f"{prefix}rewards/chosen"] = self.accelerator.gather_for_metrics(metrics_chosen_rewards).mean().item()
        metrics[f"{prefix}rewards/rejected"] = self.accelerator.gather_for_metrics(metrics_rejected_rewards).mean().item()
        metrics[f"{prefix}rewards/accuracies"] = self.accelerator.gather_for_metrics(reward_accuracies).mean().item()
        metrics[f"{prefix}rewards/margins"] = (
            self.accelerator.gather_for_metrics(metrics_chosen_rewards - metrics_rejected_rewards).mean().item()
        )
        ...
        # Log consistency weights for RGPO
        if sample_weights is not None:
            metrics[f"{prefix}consistency_weight_mean"] = (
                self.accelerator.gather_for_metrics(sample_weights).mean().item()
            )
            # Also log sum of weights to understand batch-level importance
            metrics[f"{prefix}consistency_weight_sum"] = (
                self.accelerator.gather_for_metrics(sample_weights).sum().item()
            )

        # Use simple mean for loss calculation
        # This preserves batch-level differentiation: batches with more high-weight samples
        # naturally have higher loss and contribute more to training.
        # Alternative approach (sum/weight.sum()) would normalize away batch importance.
