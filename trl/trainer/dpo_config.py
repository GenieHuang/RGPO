...
class DPOConfig(TrainingArguments):
        ...
        loss_type (`str` or `list[str]`, *optional*, defaults to `"sigmoid"`):
            Type of loss to use. Possible values are:
                ...
                - `"ipo"`: IPO loss from the [IPO](https://huggingface.co/papers/2310.12036) paper.
                  The loss is computed as: (log_ratio_chosen - log_ratio_rejected - 1/(2*beta))^2.
                  Can be combined with `use_consistency_weighted=True` to apply Dawid-Skene sample weights
                  for RGPO: |s_i| * (log_ratio - 1/(2*beta))^2.
    ...
    # Parameters for RGPO (Reliability-Guided Preference Optimization) using Dawid-Skene model
    use_consistency_weighted: bool = field(
        default=False,
        metadata={
            "help": "Whether to use consistency-weighted RGPO with annotator reliability from Dawid-Skene model. "
            "When enabled, |s_i| (confidence score) is used as sample weights."
        },
    )
    ...
    simpo_gamma: float = field(
        default=0.5,
        metadata={
            "help": "Target reward margin γ for SimPO loss. SimPO uses length-normalized log probabilities "
            "and this margin term to encourage the model to prefer chosen responses by a certain margin. "
            "The paper recommends values around 0.5-1.0. Only used when loss_type is 'simpo' or 'robust_simpo'."
        },
    )
